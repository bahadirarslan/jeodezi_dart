...
class GreatCircle {
  ...
  double distance(Coordinate startPoint, Coordinate endPoint) {
    ...
    final double a = pow(sin(dLat / 2), 2) + pow(sin(dLon / 2), 2) * cos(lat1.toRadians()) * cos(lat2.toRadians());
  ...
  Coordinate midpoint(Coordinate startPoint, Coordinate endPoint) {
    ...
    final double lat = atan2((sin(lat1) + sin(lat2)), sqrt((cos(lat1) + bX) * (cos(lat1) + bX) + bY * bY));
  ...
  /// ```
  Coordinate intermediate({required Coordinate startPoint, required Coordinate endPoint, required double fraction}) {
  ...
  Coordinate? intersection(
      {required Coordinate firstPoint, required double firstBearing, required Coordinate secondPoint, required double secondBearing}) {
    ...
    final double delta12 = 2 * asin(sqrt(sin(dLat / 2) * sin(dLat / 2) + cos(lat1) * cos(lat2) * sin(dLon / 2) * sin(dLon / 2)));

    final double cosTetaa = (sin(lat2) - sin(lat1) * cos(delta12)) / (sin(delta12) * cos(lat1));
    final double cosTetab = (sin(lat1) - sin(lat2) * cos(delta12)) / (sin(delta12) * cos(lat2));
    ...
    final double cosAlpha3 = -cos(alpha1) * cos(alpha2) + sin(alpha1) * sin(alpha2) * cos(delta12);
    final double delta13 = atan2(sin(delta12) * sin(alpha1) * sin(alpha2), cos(alpha2) + cos(alpha1) * cosAlpha3);

    final double lat = asin(min(max(sin(lat1) * cos(delta13) + cos(lat1) * sin(delta13) * cos(firstBearing.toRadians()), -1), 1));
    final double deltaLon13 = atan2(sin(firstBearing.toRadians()) * sin(delta13) * cos(lat1), cos(delta13) - sin(lat1) * sin(lat));
  ...
  /// ```
  Coordinate destination({required Coordinate startPoint, required double bearing, required double distance}) {
    final double lat1 = startPoint.latitude.toRadians();
    final double lon1 = startPoint.longitude.toRadians();

    final double sinLat = sin(lat1) * cos(distance / earthRadiusKm) + cos(lat1) * sin(distance / earthRadiusKm) * cos(bearing.toRadians());
    final double lat = asin(sinLat);

    final double y = sin(bearing.toRadians()) * sin(distance / earthRadiusKm) * cos(lat1);
  ...
  /// ```
  double crossTrackDistance({required Coordinate currentPoint, required Coordinate startPoint, required Coordinate endPoint}) {
  ...
  /// ```
  double alongTrackDistanceTo({required Coordinate currentPoint, required Coordinate startPoint, required Coordinate endPoint}) {
    if (currentPoint == startPoint) return 0;
    ...
    final double deltaAlongTrack = acos(cos(delta13) / cos(deltaCrossTrack).abs());
  ...
  /// ```
  double maxLatitude({required Coordinate startPoint, required double bearing}) {
    return acos(sin(bearing.toRadians()) * cos(startPoint.latitude.toRadians())).toDegrees();
  }
  ...
  /// ```
  List<double> crossingParallels({required Coordinate startPoint, required Coordinate endPoint, required double latitude}) {
    ...
    final double y = sin(lat1) * cos(lat2) * cos(latInRads) * cos(deltaLon) - cos(lat1) * sin(lat2) * cos(latInRads);
